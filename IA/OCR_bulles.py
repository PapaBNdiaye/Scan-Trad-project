...
def correct_text(text):
    result = corrector(text, max_new_tokens=100, num_return_sequences=1)[0]["generated_text"]
    return result
...
def get_texts(path_image, boxes, padding=0):
    ...
    # Agrandissement pour meilleure lisibilité
    ...
    sharpened = cv2.filter2D(image, -1, kernel)  # améliorer la netteté
    ...
    for index, (centerX, centerY, width, heigth) in enumerate(boxes):
        ...
        cv2.imwrite(str(index)+"test_roi.jpg", roi)
        ...
        text_final = text_corrected.split("|")[0]
        print(text_final)
        boxes[index].append(text_final)
    return boxes
